...
export async function GET(request: NextRequest) {
  try {
    ...
    const completedClasses = subscriptions.reduce((total, sub) => 
      total + sub.classSessions.length, 0
    );
    ...
    const achievements = [
      ...(totalCourses > 0 ? [{
        ...
        earnedDate: subscriptions[0]?.startDate.toISOString() || new Date().toISOString(),
