            ...
            <CardContent className='pt-6'>
              <div className={cn('flex items-center gap-3', isRTL && 'flex-row-reverse')}>
                ...
                <div className={cn(isRTL && 'text-right')}>
                  ...
                  <h3 className='text-2xl font-bold'>
                    {dashboardData?.stats.currentStreak || 0} {isRTL ? 'أيام' : 'days'}
                  </h3>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent className='space-y-6'>
                <div className='space-y-2'>
                  <div className={cn('flex justify-between', isRTL && 'flex-row-reverse')}>
                    ...
                    <span className='text-sm text-muted-foreground'>
                      {dashboardData?.progressOverview.weeklyProgress || 0}/{dashboardData?.progressOverview.weeklyGoal || 10} 
                      {isRTL ? ' حصة' : ' classes'}
                    </span>
                  </div>
                  <Progress 
                    value={((dashboardData?.progressOverview.weeklyProgress || 0) / (dashboardData?.progressOverview.weeklyGoal || 10)) * 100} 
                    className='h-3'
                  />
                </div>
                ...
                <div className='space-y-2'>
                  <div className={cn('flex justify-between', isRTL && 'flex-row-reverse')}>
                    ...
                    <span className='text-sm text-muted-foreground'>
                      {dashboardData?.progressOverview.monthlyProgress || 0}/{dashboardData?.progressOverview.monthlyGoal || 40}
                      {isRTL ? ' حصة' : ' classes'}
                    </span>
                  </div>
                  <Progress 
                    value={((dashboardData?.progressOverview.monthlyProgress || 0) / (dashboardData?.progressOverview.monthlyGoal || 40)) * 100} 
                    className='h-3'
                  />
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className='space-y-4'>
                  {dashboardData?.recentActivity.map((activity) => (
                    <div key={activity.id} className={cn('flex items-center gap-3 p-3 border rounded-lg', isRTL && 'flex-row-reverse')}>
                      ...
                      <div className={cn('text-right', isRTL && 'text-left')}>
                        <p className='text-xs text-muted-foreground'>
                          {new Date(activity.timestamp).toLocaleDateString(isRTL ? 'ar-SA' : 'en-US', {
                            month: 'short',
                            day: 'numeric',
                            hour: '2-digit',
                            minute: '2-digit'
              ...
              <CardContent>
                <div className='space-y-3'>
                  {dashboardData?.upcomingClasses.map((classItem) => (
                    <div key={classItem.id} className={cn('flex items-center gap-3 p-3 border rounded-lg hover:bg-accent transition-colors', isRTL && 'flex-row-reverse')}>
                      ...
                      <div className={cn('flex-1', isRTL && 'text-right')}>
                        ...
                        <p className='text-xs text-muted-foreground'>
                          {formatTime(classItem.time)} • {classItem.duration} {isRTL ? 'دقيقة' : 'min'}
                        </p>
                      </div>
              ...
              <CardContent>
                <div className='space-y-3'>
                  {dashboardData?.achievements.map((achievement) => (
                    <div key={achievement.id} className={cn('flex items-center gap-3 p-3 border rounded-lg', isRTL && 'flex-row-reverse')}>
                      <div className={cn(
                        'p-2 rounded-full',
                      ...
                      <div className={cn('flex-1', isRTL && 'text-right')}>
                        ...
                        <p className='text-xs text-muted-foreground'>
                          {new Date(achievement.earnedDate).toLocaleDateString(isRTL ? 'ar-SA' : 'en-US', {
                            month: 'short',
                            day: 'numeric'
                          })}
                        </p>
